...
func (r rc4md5Key) IVSize() int                       { return 16 }
func (r rc4md5Key) Decrypter(iv []byte) cipher.Stream { return r.Encrypter(iv) }
func (r rc4md5Key) Encrypter(iv []byte) cipher.Stream {
	h := md5.New()
	h.Write(r)
	h.Write(iv)
	rc4key := h.Sum(nil)
